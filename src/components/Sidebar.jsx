...
const Sidebar = ({ cryptoData }) => {
  ...
  return (
    <div className='lg:flex flex-col hidden'>
      ...
      <div className='min-h-min-content bg-white mt-6 mb-3 rounded-lg'>
        <h1 className='text-[25px] pt-6 px-8 font-semibold '>Trending Coins {'(24h)'}</h1>
        <ul className='p-4 px-8'>
          {cryptoData.map((crypto) => (
            <li key={crypto.id} className='py-2 flex items-center'>
              ...
              <span className={`percentage text-lg font-[600] ml-auto ${crypto.market_data?.current_price.usd < 0 ? 'text-red-500' : 'text-green-500'}`}>
                {crypto.market_data?.current_price.usd < 0 ? <span>&#9660;  </span> : <span>&#9650;  </span>}
